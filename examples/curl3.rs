...
struct Collector(Vec<u8>, Mutex<Progress>);
...
impl Handler for Collector {
    ...
    fn progress(&mut self, dltotal: f64, dlnow: f64, _: f64, _: f64) -> bool {
        ...
        self.1.lock().unwrap().bar(dltotal as usize, "label");
        true
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mb = Mutex::new(Progress::new());

    let mut multi = Multi::new();
    let mut handles = URLS
        .iter()
        .enumerate()
        .map(|(token, url)| Ok((token, download(&mut multi, token, url, mb)?)))
        ...
        multi.messages(|message| {
                ...
                .expect("token mismatch with the `EasyHandle`")
            {
                Ok(()) => {
                    ...
                    mb.lock()
                        .unwrap()
                        .set_and_draw(&handle.get_ref().1.lock().unwrap(), 100)
